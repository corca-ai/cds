let k = { data: "" }, O = (e) => typeof window == "object" ? ((e ? e.querySelector("#_goober") : window._goober) || Object.assign((e || document.head).appendChild(document.createElement("style")), { innerHTML: " ", id: "_goober" })).firstChild : e || k, C = /(?:([\u0080-\uFFFF\w-%@]+) *:? *([^{;]+?);|([^;}{]*?) *{)|(}\s*)/g, F = /\/\*[^]*?\*\/|  +/g, y = /\n+/g, d = (e, l) => {
  ...
  for (let t in e) {
    let a = e[t];
    t[0] == "@" ? t[1] == "i" ? n = t + " " + a + ";" : i += t[1] == "f" ? d(a, t) : t + "{" + d(a, t[1] == "k" ? "" : l) + "}" : typeof a == "object" ? i += d(a, l ? l.replace(/([^,])+/g, (r) => t.replace(/(^:.*)|([^,])+/g, (s) => /&/.test(s) ? s.replace(/&/g, r) : r ? r + " " + s : s)) : t) : a != null && (t = /^--/.test(t) ? t : t.replace(/[A-Z]/g, "-$&").toLowerCase(), o += d.p ? d.p(t, a) : t + ":" + a + ";");
  }
  ...
  let t = j(e), a = p[t] || (p[t] = ((s) => {
    let c = 0, u = 11;
    for (; c < s.length; )
      u = 101 * u + s.charCodeAt(c++) >>> 0;
    return "go" + u;
  })(t));
  if (!p[a]) {
    let s = t !== e ? e : ((c) => {
      let u, g, f = [{}];
      for (; u = C.exec(c.replace(F, "")); )
        u[4] ? f.shift() : u[3] ? (g = u[3].replace(y, " ").trim(), f.unshift(f[0][g] = f[0][g] || {})) : f[0][u[1]] = u[2].replace(y, " ").trim();
      return f[0];
    })(e);
...
//# sourceMappingURL=goober.modern.mjs.map
